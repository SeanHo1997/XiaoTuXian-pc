      ...
      <template v-else>
        ...
        <div class="form-item">
          <div class="input">
            ...
            <span class="code">{{ time === 0 ? '发送验证码' : '${time}秒后发送' }}</span>
  ...
  setup () {
    ...
    const login = async () => {
      ...
      if (valid) {
        ...
        // 成功
        // 1. 存储信息
        const { id, account, nickname, avatar, token, mobile } = data.value.result
        store.commit('user/setUser', { id, account, nickname, avatar, token, mobile })
        // 2. 提示
        Message({ type: 'success', text: '登录成功' })
        // 3. 跳转
    ...
    return { isMsgLogin, form, scheam: mySchema, login, send, time }
  }
}
</script>
