      ...
      <dd>
        <template>
          <div v-for="(val) in item.values" :key="val.name">
            <img :class="{selected: val.selected}" @click="changeSku(val)" v-if="val.picture" :src="val.picture" :title="val.name" alt="">
            <span :class="{selected: val.selected}" @click="changeSku(val)" v-else>{{ val.name }}</span>
          </div>
        </template>
      </dd>
    </dl>
